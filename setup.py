...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_rbs", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_rbs": ["*.pyi", "py.typed"],
        "tree_sitter_rbs.queries": ["*.scm"],
    },
    ext_package="tree_sitter_rbs",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_rbs/binding.c",
                "src/parser.c",
            ],
            define_macros=[
